...
public class AppListAdapter extends RecyclerView.Adapter<AppListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.materialSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if(isChecked)
                excludedAppMap.put(resolveInfoList.get(holder.getAdapterPosition()).activityInfo.packageName,"");
            else excludedAppMap.remove(resolveInfoList.get(holder.getAdapterPosition()).activityInfo.packageName);
        });
