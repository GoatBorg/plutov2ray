...
import static com.bycomsolutions.bycomvpn.utils.Utils.extractAverageRTT;
...
public class SpeedTestActivity extends AppCompatActivity{
    ...
    private void getOptimalServer() {
        ...
        new Handler().postDelayed(() -> {
            ...
            if(Server_Location.isEmpty()){
                Server_Location = getString(R.string.fastest_server);
    ...
    private void runPingTest() {
            ...
            do {
                try {
                    ...
                    if (address.isReachable(5000)) {
                        ...
                        long pingTime = endTime - startTime;
                        totalPingTime += pingTime;
                        long averageTime = totalPingTime / successfulPings;
                        averagePing.set(averageTime + " ms");

                        Log.e("sdklfjsdlkfj",pingTime+"");

                    }
                } catch (IOException ignored) {}
            } while (!isFinished.get() && SpeedTestRunning && successfulPings<10);
        });

        new Handler().postDelayed(() -> {
            isFinished.set(true);
            onPingTestFinished(averagePing.get());
        },3000);
       ...
       /* new Thread(() -> {
            ...
            runOnUiThread(() -> onPingTestFinished(finalResult1));


            Log.e("dsfjksdljkf",extractAverageRTT(result));
...
*/
        ...
        for (int i = 0; i<THREADS;i++) {
            new Thread(() -> {
                try {
                    ...
                    Log.e("dfksdjfklsdjfsfldjk",downloadUrl.toString());
    ...
    private String generateDownloadURL(){
       // String randomHash = Integer.toHexString(new SecureRandom().nextInt());
        return Server_URL + "download?size=2500000000";
    }
    ...
    @Override
    public void finish(){
        if(SpeedTestRunning) SpeedTestRunning = false;
        super.finish();
    }
